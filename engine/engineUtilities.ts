 ...
 *  @return {Number}
 *  @memberof Utilities */
const abs = (a: any) => a < 0 ? -a : a;

/** Returns the sign of value passed in
 *  @param {Number} value
 *  @return {Number}
 *  @memberof Utilities */
const sign = (a: any) => a < 0 ? -1 : 1;
...
const clamp = (v: any, max=1, min=0)=> (ASSERT(max > min), v < min ? min : v > max ? max : v);
 ...
 *  @return {Number}
 *  @memberof Utilities */
const percent = (v: any, max=1, min=0)=> max-min ? clamp((v-min) / (max-min)) : 0;
 ...
 *  @return {Number}
 *  @memberof Utilities */
const lerp = (p: any, max=1, min=0)=> min + clamp(p) * (max-min);

/** Formats seconds to 00:00 style for display purposes 
 *  @param {Number} t - time in seconds
 *  @return {String}
 *  @memberof Utilities */
const formatTime = (t: any) => (t/60|0)+':'+(t%60<10?'0':'')+(t%60|0);
...
const nearestPowerOfTwo = (v: any) => 2**Math.ceil(Math.log2(v));
...
const isOverlapping = (pA: any, sA: any, pB: any, sB: any)=> abs(pA.x - pB.x)*2 < sA.x + sB.x & abs(pA.y - pB.y)*2 < sA.y + sB.y;
 ...
 *  @return {Color}
 *  @memberof Random */
const randColor = (cA = new Color, cB = new Color(0,0,0,1), linear: any)=>
...
const vec2 = (x=0, y: any)=> x.x == undefined ? new Vector2(x, y == undefined? x : y) : new Vector2(x.x, x.y);
